...
const metadata:NFTtokenMetadata=new NFTtokenMetadata(
 "string", // ex. "Arch Nemesis: Mail Carrier" or "Parcel #5055"
  ...
  "1", // number of copies of this set of metadata in existence when token was minted.
  ...
  "string", // anything extra the NFT wants to store on-chain. Can be stringified JSON.
  "string", // URL to an off-chain JSON file with more info.
  "string" // Base64-encoded sha256 hash of JSON from reference field. Required if `reference` is included.
);
...
export function nft_mint( receiver_id: string, token_id: string):void{
