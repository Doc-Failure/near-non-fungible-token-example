...
export function nft_mint( token_id: string, metadata: TokenMetadata, receiver_id: string):void{
    nft_mint_impl( token_id, metadata, receiver_id);
}

export function nft_metadata():NFTContractMetadata{
    const res: NFTContractMetadata = new NFTContractMetadata("nft-1.0.0", "name", "symbol", "icon", "base_uri", "reference", "reference_hash");
    return res;
}

  // Query for all the tokens for an owner
  export function nft_tokens_for_owner( account_id: string, from_index: u128|null=u128.Zero, limit: u128|null=new u128(50) ): Array<Token>{
  ...
  /*   let tokens_for_owner_set: PersistentSet<Token>|null = this.tokens_per_owner.get(account_id);
    ...
    let token = new Token("1", "doc_failure.testnet", metadata);
    let tokenxO:Array<Token>= new Array<Token>();
    tokenxO.push(token);
    return tokenxO;
  }
